...
int isBetween(int x, int h, int l) {
    ...
    int x_sign = (x >> 31) & 1;
    int sgn_minus =((h + (~x + 1)) >> 31) & 1;


    int var = (h >> 31) ^(x >> 31);
    int hGreaterx = (!!(h^x)) & ((var & x_sign) | (!(var | (sgn_minus))));
    
    int l_sign = (l >> 31) & 1;   
    int sgn_minus2 =((x + (~l + 1)) >> 31) & 1;

    int var2 = (x >> 31) ^(l >> 31);
    int xGreaterl = (!!(x^l)) & ((var2 & l_sign) | (!(var2 | (sgn_minus2))));
  
	return(hGreaterx & xGreaterl);
...
int bitSwap(int x, int n, int m) {
	...
	int conditional, swapped, mInRange, nInRange;
	int nBit, mBit, value, a, b;
	a = (  n ^ (  n & 0)) >> 31;
    b = ((31 + ~n+1) ^ ((31 ^ n) & ((31 + ~n+1) ^ 31))) >> 31;
    nInRange = 1 & ~(a | b);
        a = (  m ^ (  m & 0)) >> 31;
        b = ((31 + ~m+1) ^ ((31 ^ m) & ((31 + ~m+1) ^ 31))) >> 31;
        mInRange = 1 & ~(a | b);

	 conditional = ((nInRange&mInRange)<<31)>>31;

	
	  
	  nBit= (x>>n)&1;
	  mBit = (x>>m)&1;
	  value = ((nBit ^ mBit) << n) | ((nBit ^ mBit) << m);
     swapped = x ^ value;
	
	
    return ((conditional & swapped) | (~conditional & x)    );
...
int logicalOr(int x, int y) {
	...
	int isXNotZero = (((x | (~x + 1)) >> 31) & 1);
	int isYNotZero = (((y | (~y + 1)) >> 31) & 1);

    return ( isXNotZero|isYNotZero );
}
/* 
...
int rotate(int x, int w) {
    ...
    int wTRUE = ~w+1;
    int leftmost = (x>>31)&1;
	int leftRotate = (x<<1) + leftmost;
	
    int rightRotate = ( (x >> 1) & ~(~0 << 31)) | ( x << 31);

    return  (wTRUE & leftRotate) | (~wTRUE & rightRotate) ;
}
/* 
...
int twocomp2onecomp(int x) {
  ...
  int isTMIN = (~(!( (!x) | (x + x) ))) +1;
   //int onesCompVal = (x+(~0));
  int onesCompVal = (((isPos) & ((x+(~0)))) | ((~isPos) & ((x+(~0)+1))) );
  
  return (((isTMIN) & (~0)) | ((~isTMIN) & onesCompVal) );
...
unsigned float_range(unsigned uf) {
	...
	int frac_size = 23;
	int exp_size = 8;
	int frac_mask =((1<<frac_size)-1);
    int exp_mask =((1<<exp_size)-1);

	int expo = (uf>>frac_size) & exp_mask;
	int frac = uf & frac_mask;
	

	
	            //.999999 .00001                1.0 & -1.0
	if ( (expo == 0x7e && frac != 0) || (expo == 0x7f && frac == 0) || expo == 0 || expo == 1   )
		return 1;
...
unsigned float_gtOne(unsigned uf) {
	...
	int expo = (uf>>23) & exp_mask;
	int frac = uf & frac_mask;
	int sign = (uf>>(32-1)) & 0x1;
	
	
	if((expo == 0x7f && frac != 0 && sign == 0) || ( expo > 0x7f  && sign !=1 && expo < 0xff)  
	    || (expo == 0xff && frac == 0 && sign != 1)     )     {
		return 1;
	}
